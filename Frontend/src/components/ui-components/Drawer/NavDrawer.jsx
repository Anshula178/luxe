            ...
            {menuItems.map((item, index) => (
              <li key={index} className="relative group">
                ...
                >
                  <Link
                    to={item.link}
                    className="block text-lg text-black transition duration-300 relative hover:text-orange-600 font-outfitMedium"
                    onClick={onClose}
                  >
                    {item.name}
                  </Link>
                  {item.subMenu && (
                    <ChevronDown
                      size={16}
                      className="ml-2  text-black group-hover:text-orange-600"
                    />
                  )}
                </div>
                {/* Submenu */}
